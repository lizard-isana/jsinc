...
JavaScriptで外部のhtmlファイルなどをインクルードするためのライブラリです。

- インクルードファイル内のスクリプトタグ（直接記述、リンクによる埋め込み）が動作します
- インクルードファイル内のスタイシート（直接記述、リンクによる埋め込み）が動作します
- ShadowDOMとして、親ページとは独立したDOM内にコンテンツを埋め込むことができます
- API(Hook)を利用して、ファイルを読み込む各プロセスに介入できます
...
このわずかな時間差は、通常のHTMLを表示する場合にはほとんど問題になりませんが、スクリプトなどで、読み込み後のページの内容を操作する場合には、意図した挙動にならない場合があります。読み込まれたインクルード部分を操作したい場合や、インクルードが読み込まれた後のページ全体を操作したい場合は、別項で説明するHookなどを利用して、適切なタイミングでスクリプトが動作するように設定する必要があります。

また、ヘッダタグ内で使用した場合に、metaタグなどページが読み込まれた時点で効力を発揮するものは実際には機能しない可能性があります（タグとしては挿入されるものの、ブラウザやクローラに値が渡らない）。
...
## セキュリティ
JSIncは、初期化時やファイルの読み込み時のパラメータや読み込まれるファイルの内容に対して暗黙のバリデーションやサニタイズを行いません。ユーザーの入力やURLクエリを、初期化時のパラメータや読み込みファイルのURLとして使用したり、コンテンツとして要素内に展開するような使用にはクロスサイトスクリプティングなどのセキュリティリスクが生じます。
